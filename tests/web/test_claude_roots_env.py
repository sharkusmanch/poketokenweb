...
from poketokenweb import claude_roots


def test_unset_env_yields_no_roots():
    assert claude_roots.configured_roots(env={}) == []


def test_single_root_is_read(tmp_path):
    assert claude_roots.configured_roots(
        env={claude_roots.PROJECT_ROOTS_ENV: str(tmp_path)}
    ) == [tmp_path]


def test_several_roots_are_split_on_pathsep(tmp_path):
    a, b = tmp_path / "a", tmp_path / "b"
    roots = claude_roots.configured_roots(
        env={claude_roots.PROJECT_ROOTS_ENV: os.pathsep.join([str(a), str(b)])}
    )
    assert roots == [a, b]
...
def test_blank_entries_are_skipped(tmp_path):
    ...
    roots = claude_roots.configured_roots(
        env={claude_roots.PROJECT_ROOTS_ENV: f"{tmp_path}{os.pathsep}"}
    )
...
def test_surrounding_whitespace_is_stripped(tmp_path):
    roots = claude_roots.configured_roots(
        env={claude_roots.PROJECT_ROOTS_ENV: f"  {tmp_path}  "}
    )
    assert roots == [tmp_path]
...
def test_tilde_is_expanded():
    roots = claude_roots.configured_roots(
        env={claude_roots.PROJECT_ROOTS_ENV: "~/somewhere"}
    )
...
def test_missing_roots_are_reported(tmp_path):
    ...
    assert claude_roots.missing_roots([present, absent]) == [absent]
...
def test_a_file_is_reported_as_missing(tmp_path):
    ...
    assert claude_roots.missing_roots([not_a_dir]) == [not_a_dir]
...
def test_relative_entries_are_made_absolute(tmp_path, monkeypatch):
    ...
    (tmp_path / "rel").mkdir()
    roots = claude_roots.configured_roots(env={claude_roots.PROJECT_ROOTS_ENV: "rel"})
...
def test_overlapping_roots_do_not_double_count(tmp_path):
    ...
    roots = claude_roots.configured_roots(
        env={claude_roots.PROJECT_ROOTS_ENV: os.pathsep.join([str(outer), str(outer / "-proj")])}
    )
